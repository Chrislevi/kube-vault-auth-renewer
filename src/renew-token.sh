...
while true
do
    TOKEN_LOOKUP_RESPONSE=$(curl -sS --insecure  \
    ...
    if [ ${CURRENT_TTL} -lt ${RENEWAL_TTL_THRESHOLD} -o ${CURRENT_TTL} -lt ${RENEW_INTERVAL_TTL_THRESHOLD} ]; then
        ...
        TOKEN_RENEW=$(curl -sS --insecure  --request POST \
          --header "X-Vault-Token: ${VAULT_TOKEN}" \
    ...
    do
        LEASE_LOOKUP_RESPONSE=$(curl -sS --insecure  --request PUT \
          --header "X-Vault-Token: ${VAULT_TOKEN}" \
        ...
        if [ ${CURRENT_TTL} -lt ${RENEW_INTERVAL_TTL_THRESHOLD} ]; then
            ...
            SECRET_RENEW=$(curl -sS --insecure  --request PUT \
              --header "X-Vault-Token: ${VAULT_TOKEN}" \
